...
"ind.plots.cwres.qq" <-
  function(object,
    ...
    if(is.null(check.vars(c("id","cwres"),object))) {
      cat("The ID and CWRES variables all need to be defined in the current database!\n")
      return(NULL)
    }
    ...
    for (i in id.levels) {    ## start loop
      ...
      xplot <- xpose.plot.qq(xvardef("cwres",nobj),
                             new.obj,
                                        #xlb = xlb,
                                        #ylb = ylb,
                             by=xvardef("id",nobj),
    ...
    obj <- xpose.multiple.plot(plotList,max.plots.per.page=max.plots.per.page,plotTitle=NULL,...)
    return(obj)
