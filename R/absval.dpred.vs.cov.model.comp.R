...
"absval.dpred.vs.cov.model.comp" <-
  function(object, 
    ...
    if ((is.null(xvardef("idlab",object))) || (is.null(xvardef("pred",object)))) {
      cat("The required variables (ID label, PRED) are not set in the database!\n")
      return()
    } 
    ...
    object@Data$dPRED <- abs(object@Data[,xvardef("pred", object)] - object.ref@Data[,xvardef("pred", object.ref)])
    ...
    ## |dPRED| vs covariates
    default.plot.title <- paste("|PRED_(Run", object@Runno,
                                ") - PRED_(Run",object.ref@Runno,
                                ")| vs. Covariates",sep="")
