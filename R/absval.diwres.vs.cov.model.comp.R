...
"absval.diwres.vs.cov.model.comp" <-
  function(object, 
    ...
    if ((is.null(xvardef("idlab",object))) || (is.null(xvardef("iwres",object)))) {
      cat("The required variables (ID label, IWRES) are not set in the database!\n")
      return()
    } 
    ...
    object@Data$dIWRES <- abs(object@Data[,xvardef("iwres", object)] -
                             object.ref@Data[,xvardef("iwres", object.ref)])
    ...
    ## |dPRED| vs covariates
    default.plot.title <- paste("|IWRES_(Run", object@Runno,
                                ") - IWRES_(Run",object.ref@Runno,
                                ")| vs. Covariates",sep="")
